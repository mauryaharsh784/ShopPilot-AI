...
export async function addCartHandler(req: Request, res: Response) {
  const body = z.object({ productId: z.string(), quantity: z.number().int().min(1).optional() }).parse(req.body);
...
export async function updateCartHandler(req: Request, res: Response) {
  const body = z.object({ productId: z.string(), quantity: z.number().int() }).parse(req.body);
  res.json(await updateCart(req.user!.id, body.productId, body.quantity));
}
...
export async function removeCartHandler(req: Request, res: Response) {
  ...
  await trackEvent({ userId: req.user!.id, eventType: "remove_from_cart", productId: req.params.productId });
  res.json(cart);
}
...
export async function clearCartHandler(req: Request, res: Response) {
  ...
  if (!body.success) throw badRequest("Confirmation required to clear the cart");
  res.json(await clearCart(req.user!.id));
}

export async function checkoutHandler(req: Request, res: Response) {
  await trackEvent({ userId: req.user!.id, eventType: "checkout_started" });
  const order = await checkout(req.user!.id);
  res.status(201).json({ order, notice: "Demo checkout only — no payment was captured." });
}
...
export async function getOrderHandler(req: Request, res: Response) {
  res.json({ order: await getOrder(req.user!.id, req.params.id, req.user!.role) });
}

export async function getPrefsHandler(req: Request, res: Response) {
  res.json({ preferences: req.user!.preferences });
}

export async function putPrefsHandler(req: Request, res: Response) {
  const body = z
    .object({
...
export async function recommendationsHandler(req: Request, res: Response) {
  ...
  const result = await listProducts({
    category: typeof req.query.category === "string" ? req.query.category : prefs?.categories[0],
    maxPrice: req.query.budget ? Number(req.query.budget) : prefs?.budget,
    limit: 8,
  });
  res.json(result);
}
