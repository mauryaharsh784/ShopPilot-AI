...
export const api = {
  ...
  compare: (productIds: string[]) =>
    request<{ products: Product[] }>("/api/products/compare", {
      method: "POST",
      body: JSON.stringify({ productIds }),
    }),

  recommendations: () =>
    request<Paged<Product>>("/api/recommendations"),

  cart: () => request<CartView>("/api/cart"),

  addToCart: (productId: string, quantity = 1) =>
    request<CartView>("/api/cart", {
      method: "POST",
      body: JSON.stringify({
        productId,
        quantity,
  ...
  removeFromCart: (productId: string) =>
    request<CartView>(`/api/cart/${productId}`, {
      method: "DELETE",
    }),

  clearCart: () =>
    request<CartView>("/api/cart/clear", {
      method: "POST",
      body: JSON.stringify({
        confirm: true,
      }),
  ...
  chat: (body: {
    ...
    request<AgentChatResult>("/api/agent/chat", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  dashboard: () =>
    request<DashboardData>("/api/analytics"),
